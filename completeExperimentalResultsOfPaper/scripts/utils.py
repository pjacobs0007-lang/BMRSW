...
def sample_gandk_outl_v2(n, d, theta, eps, z_val, rho=None):
    cont_size = int(np.floor(eps * n))
    n_real = n - cont_size
    ...
    # Apply discretization logic: g(X) = floor(X / rho) * rho
    if rho is not None and rho > 0:
        sample = np.floor(sample / rho) * rho
        
    if cont_size != 0:
        outl = np.ones((cont_size, d)) * z_val
