...
# JAX ops index/index_update removed for compatibility
...
class g_and_k_model():
    ...
    def generator(self, z, theta):
        ...
        b = theta[1]
        g = theta[2]
        k = theta[3]  # Now on absolute scale
    ...
    def sample(self,theta,key):          
      ...
      z = random.normal(key, shape=(self.m, self.d))
...
class toggle_switch_model():
    ...
    def generator_single(self,theta,uvals):
        ...
        for t in range(0,self.T-1):
            ...
            u = u.at[t+1].set(u_new+0.5*jstats.norm.ppf(phi_u_new+uvals[t]*(1.-phi_u_new)))

            v_new = v[t] +(alpha2/(1.+(u[t]**beta2)))-(1.+0.03*v[t])
            phi_v_new = jstats.norm.cdf(-2.*v_new)
            v = v.at[t+1].set(v_new+0.5*jstats.norm.ppf(phi_v_new+uvals[self.T+t]*(1.-phi_v_new)))
